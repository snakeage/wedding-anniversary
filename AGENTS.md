...
- **Scope:** the user names the screen and what must stay; the agent names files in Plan and does not edit until they agree. Do not touch neighboring screens unasked. Details: [`.cursor/rules/scope.mdc`](.cursor/rules/scope.mdc).
- **Review:** after a PR that touches cabinet APIs, RSVP, auth, payment, event schema, or a guest skin, remind the user to run `/review-bugbot` in a **new** chat. Copy/CSS/docs skip that. Merge only on «вливай». Details: [`.cursor/rules/review.mdc`](.cursor/rules/review.mdc).
