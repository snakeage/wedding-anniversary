...
`/` is the sales catalog. Live demos: Quiet luxury (adult birthday) at `/sofia`, Paper envelope (wedding) at `/ivan-maria`, Dark editorial (evening gala) at `/kira`, Garden daylight (garden wedding) at `/olga-nikita`. What to build next is **not** this paragraph — it is the issue labeled `now`: [GitHub Issues](https://github.com/snakeage/wedding-anniversary/issues) and [how we work](./backlog.md).
