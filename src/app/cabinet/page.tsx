  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      ...
      <div className="mt-8 flex flex-wrap items-center gap-6">
        <Link href="/cabinet/new" className="btn-gold">
          Создать приглашение
        </Link>
      ...
      {events.length === 0 ? (
        <p className="panel mt-10 px-6 py-10 text-center text-ink/60">Пока нет событий.</p>
      ) : (
        <ul className="mt-10 space-y-3">
          {events.map((item) => (
            <li key={item.id} className="panel px-5 py-5">
              ...
              <p className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-[10px] tracking-[0.2em] text-ink/45 uppercase">
                <Link className="transition-colors hover:text-burgundy/75" href={`/cabinet/${encodeURIComponent(item.slug)}/edit`}>
                  Изменить
                </Link>
                <Link className="transition-colors hover:text-burgundy/75" href={`/${item.slug}`}>
                  {item.status === "active" ? "Открыть как гость" : "Предпросмотр"}
                </Link>
                {item.status === "active" ? (
                  <Link
                    className="transition-colors hover:text-burgundy/75"
                    href={`/rsvp-list?slug=${encodeURIComponent(item.slug)}`}
                  >
                    Ответы гостей
                  </Link>
                ) : (
                  <span>Сбор ответов закрыт</span>
                )}
              </p>
              <CabinetPayPanel
                slug={item.slug}
